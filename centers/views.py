...
class CenterReviewDetailUpdateDeleteAPIView(APIView):
    ...
    def get(self, request, pk):
        center_review = CenterReview.objects.all()
        paginator = PageNumberPagination()
        page_obj = paginator.paginate_queryset(center_review, request)
...
class CenterList(LoginRequiredMixin, View):
    def get(self, request):
        center = Center.objects.order_by('create_at')
        search_query = request.GET.get('q')
        if search_query:
            centers = center.filter(title__icontains=search_query)
        else:
            centers = center.none()  # Initialize with an empty queryset
        ...
        return render(request, 'center/center_list.html', {'centers': centers})
